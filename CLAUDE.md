...
- 버전 규칙: 최초 릴리즈 `v1.0.0` 고정. 이후 **X = `breaking:` / Y = `feat:` / Z = `fix`·`refactor`·`perf`·`style`·`ci`** (일반 SemVer와 정의가 다름)
- 릴리즈 불가: `feat`/`fix`/`refactor`/`perf`/`style`/`ci`/`breaking` 가 하나도 없으면 릴리즈 진행 안 함
...
| 배포 계획 (버전 정책 · 플로우 · 에이전트 규칙) | `.claude/plans/deployment-plan.md` |
