...
#### 왜 Edge Function `delete-account` 가 필요한가

> 회원탈퇴는 "내 데이터를 완전히 지우는" 액션이지만, 구조상 **클라이언트 권한만으로는 완결할 수 없다.** 그래서 서버 권한으로 수행해야 하는 부분만 따로 떼어낸 전용 엔드포인트가 `delete-account` Edge Function 이다.

##### 1. 왜 필요한가 — 서버 권한이 필요한 작업이 있기 때문

- `auth.users` row 삭제는 **`service_role` 키로만 가능**하다. `authenticated` 역할(= 로그인한 클라이언트)은 본인 row조차 직접 `DELETE` 할 수 없다 — `auth` 스키마는 RLS 로 보호되어 있고 admin API 경유를 강제하기 때문.
- `service_role` 키는 모든 RLS 를 무시하고 DB 전체에 대한 관리자 권한을 갖는다. 이 키를 **Flutter 앱 번들에 포함시키는 순간** 리버스 엔지니어링으로 누구나 다른 사용자 데이터를 읽고 쓸 수 있게 된다 — 치명적 보안 사고. 따라서 `service_role` 은 **반드시 서버 측(Supabase Edge Function / 자체 백엔드)** 에만 존재해야 한다.
- DB trigger 는 **능동적으로 `auth.users` 를 DELETE 할 수 없다.** trigger 는 "row 가 변경될 때 반응" 하는 수동적 훅일 뿐, "지금 이 사용자를 삭제해라" 라는 명령을 받아 수행하는 주체가 아니다. 회원가입 시 `handle_new_user` trigger (01_auth.md) 가 동작하는 건 `INSERT ON auth.users` 이벤트를 가로채는 수동 반응이고, 탈퇴는 거꾸로 **DELETE 를 누가 쏴야 하는가** 의 문제다 — 이 "쏘는 주체" 역할을 Edge Function 이 맡는다.
- 요약: `auth.users` 삭제는 (a) `service_role` 권한이 필요하고, (b) 그 권한을 클라이언트에 둘 수 없으며, (c) trigger 로도 대체할 수 없다 → **서버 측 엔드포인트가 유일한 해결책**.

##### 2. 회원탈퇴와의 연관 — 어느 부분을 Edge Function 이 담당하는가

회원탈퇴 전체 흐름을 "클라이언트가 처리 가능한 부분" 과 "서버 권한이 필요한 부분" 으로 나누면:

| 단계 | 작업 | 권한 | 수행 주체 |
|------|------|------|-----------|
| 1 | Storage `profile-images/<userId>/` 폴더 정리 | Storage RLS (본인 폴더) | 클라이언트 |
| 2 | `profiles` row 삭제 | RLS (`id = auth.uid()`) | 클라이언트 |
| 3-a | `deleted_accounts` INSERT (쿨다운 기록) | 일반적으로 RLS 로 허용 가능하나, 3-b 와 **원자성 보장**을 위해 서버에서 함께 처리 | Edge Function |
| 3-b | `auth.users` DELETE (→ `todos` cascade) | `service_role` | Edge Function |
| 4 | `auth.signOut()` + router redirect | 클라이언트 세션 | 클라이언트 |

- 단계 3-a 와 3-b 는 **반드시 한 묶음으로 성공/실패해야 한다.** `auth.users` 를 먼저 지웠다가 쿨다운 기록이 실패하면, 사용자가 14일 제한 없이 즉시 재가입해 버릴 수 있다. 반대로 쿨다운만 찍히고 `auth.users` 삭제가 실패하면 유령 상태의 사용자가 남는다.
- Edge Function 은 이 두 작업을 **같은 실행 컨텍스트에서 순차 수행** (쿨다운 INSERT 먼저, 성공 시에만 `deleteUser` 호출) 하여 "절반만 완료된 탈퇴" 를 최소화한다. INSERT 가 실패하면 `auth.users` 는 아직 살아 있으므로 사용자가 재시도할 수 있다.
- 단계 1, 2 는 RLS 로 안전하게 클라이언트가 수행 가능하므로 Edge Function 으로 옮기지 않는다. 네트워크 한 번 호출로 끝낼 수 있는 일을 서버에 떠넘기면 Edge Function 의 cold start / timeout 리스크만 커진다.

##### 3. 처리 흐름 — Edge Function 실행 시 내부에서 일어나는 일

클라이언트가 `supabase.functions.invoke('delete-account')` 로 Edge Function 을 호출하면 Supabase SDK 가 자동으로 현재 세션의 JWT 를 `Authorization: Bearer <jwt>` 헤더로 실어 보낸다. Edge Function 은 다음 순서로 처리한다:

1. **JWT 검증** — `Authorization` 헤더 유무 확인 (없으면 401). 헤더를 그대로 단 userClient(`SUPABASE_ANON_KEY` 사용) 를 만들고 `auth.getUser()` 로 토큰을 검증 — 만료/위조 토큰이면 Supabase 가 오류 반환하므로 401 로 응답. **이 단계가 "요청자가 정말 그 사용자 본인인지" 를 확인하는 유일한 지점이다.** 클라이언트가 URL 파라미터로 `user_id` 를 보내는 방식은 위조 가능하므로 쓰지 않는다 — JWT 에서 신원을 뽑는다.
2. **identity 추출** — `user.identities[0]` 에서 `provider` (예: `google`), `provider_user_id` (Google OAuth sub), `email` 을 뽑는다. 이 값들이 `deleted_accounts` 에 기록되어 나중에 쿨다운 판정의 키가 된다. 이메일만으로는 사용자가 Google 계정 이메일을 바꾸면 우회될 수 있으므로 `provider_user_id` 를 함께 저장한다.
3. **admin client 생성** — `SUPABASE_SERVICE_ROLE_KEY` 로 adminClient 를 별도로 만든다. 이 키는 **Edge Function 환경변수에만 존재** 하며 코드/응답 어디에도 노출되지 않는다. userClient 와 adminClient 를 분리하는 이유는 "검증(user 맥락) ↔ 실행(admin 맥락)" 을 명시적으로 구분하기 위함 — admin 키로 getUser 를 부르면 아무 토큰이나 통과해 버린다.
4. **`deleted_accounts` INSERT** — adminClient 로 쿨다운 row 를 먼저 기록한다. `email`, `provider`, `provider_user_id` 를 저장. 이 INSERT 가 실패하면 `auth.users` 는 건드리지 않고 500 응답 → 클라이언트는 SnackBar 노출 후 재시도 가능 (사용자 데이터 손실 없음).
5. **`auth.admin.deleteUser(user.id)` 호출** — `auth.users` row 가 삭제된다. `todos` 테이블의 `user_id` FK 가 `on delete cascade` 이므로 **사용자의 모든 todo 가 같은 트랜잭션에서 자동 삭제** 된다. `profiles` 는 단계 2 에서 이미 클라이언트가 지웠으므로 cascade 대상이 없다.
6. **200 OK 반환** — `{ ok: true }` JSON 응답. 클라이언트 ViewModel 이 이어서 `auth.signOut()` 호출 → Supabase 가 `authStateChanges` 로 `SIGNED_OUT` 이벤트 발행 → `core/router.dart` 의 전역 redirect 가 감지해 `/login` 으로 이동. View 는 다이얼로그 pop 만 하면 된다 (직접 `go('/login')` 호출 금지 — 이중 이동 방지).

> **재시도 안전성**: 단계 4 까지만 성공하고 단계 5 가 실패하면 `auth.users` 는 살아 있지만 `deleted_accounts` 에 쿨다운이 이미 찍혀 있다. 사용자가 다시 "회원탈퇴" 를 누르면 Edge Function 이 또 호출되고, 이번엔 `deleted_accounts` 에 중복 row 가 들어갈 수 있다 — 이 때문에 `deleted_accounts` 에는 unique 제약을 걸지 않는다 (가장 최근 row 만 `check_signin_cooldown` 이 참조하므로 중복이 쌓여도 무해). `auth.admin.deleteUser` 는 멱등하므로 재호출 시 "이미 없음" 을 200 으로 swallow 하거나 그대로 삭제된다.
