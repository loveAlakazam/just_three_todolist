...
- 없으면 최초 릴리즈로 판단하고 `v1.0.0` 을 후보로 삼는다.
...
본 프로젝트 버전 규칙 (일반 SemVer 와 정의 다름):

| 자리 | 의미 | 트리거 |
|------|------|-------|
| **X** | 신규 기능 | PR 중 하나라도 `feat:` prefix 이거나 새 feature 단위 추가 |
| **Y** | 기존 기능 변경 | `feat` 은 없고 `refactor` / `style` / `perf` 등 기존 기능 동작/스펙 변경이 있음 |
| **Z** | 버그 픽스 | `fix:` prefix 만 있거나 문서/설정 변경만 있음 |

#### 결정 알고리즘

```text
if   (PR 에 feat 있음)                       → X += 1, Y = 0, Z = 0
elif (PR 에 refactor/style/perf 있음)        → Y += 1, Z = 0
else                                         → Z += 1
```

- 판정된 후보 버전을 **반드시 사용자에게 제시하고 확인**한 뒤 진행한다.
- 최초 릴리즈(태그 없음)는 `v1.0.0`.
