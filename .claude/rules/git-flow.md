...
| 자리 | 의미 | 올리는 경우 |
|------|------|-------------|
| **X** | 신규 기능 | 새로운 feature 단위가 추가되었을 때 |
| **Y** | 기존 기능의 변경 | 기존 기능의 동작/스펙이 바뀌었을 때 |
| **Z** | 버그 픽스 | 기존 기능의 버그 수정만 포함된 경우 |

> ⚠️ 본 프로젝트의 `X.Y.Z` 정의는 일반 SemVer 와 약간 다르다. **X = 신규 기능 / Y = 기존 기능 변경 / Z = 버그 픽스** 로 해석한다.
